...
{{/*
Render Kubernetes feature gates as a deterministic command-line argument value.
*/}}
{{- define "slurm-bridge.featureGates" -}}
{{- $featureGates := . -}}
{{- $items := list -}}
{{- range $name := keys $featureGates | sortAlpha -}}
{{- $items = append $items (printf "%s=%v" $name (index $featureGates $name)) -}}
{{- end -}}
{{- join "," $items -}}
{{- end }}
